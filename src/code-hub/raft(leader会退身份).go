...
func (rf *Raft) ticker() {
	for rf.killed() == false {
		...
		switch rf.state {
		case leader:
			if rf.yes_vote < (len(rf.peers) + 1) / 2 {
				rf.turn2Follower(rf.me, rf.currentTerm, true)
			} else {
				rf.broadcastEntries(true)
				rf.timerReset(append_entries_timeout)
			}
...
func (rf *Raft) replicateEntries(peer int) bool {
	rf.mu.Lock()
